...
class Compile {
  ...
  compileElement(node) {
    ...
    Array.from(nodeAttrs).forEach(attr => {
      ...
      if (this.isDirective(attrName)) {
        ...
        // 执行指令
        this.handleEvent && this.handleEvent(node, exp, dir)
      }
  ...
  handleEvent(node, exp, dir) {
    this.update(node, exp, dir, 'event')
  }

  // 所有动态绑定都需要创建更新函数以及对应watcher实例
  update(node, exp, dir, isEvent) {
    // textUpdater()
    // 初始化
    ...
    // 初始化函数方法
    isEvent && this.handleEventUpdater(node, exp, dir)
    // 更新: 
  ...
  //暗号：冬瓜冬瓜我是西瓜
  handleEventUpdater(node, exp, dir) {
    const vm = this.$vm;
    node.addEventListener(dir, vm.$methods[exp].bind(vm))
  }
